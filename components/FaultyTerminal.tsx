...
const FaultyTerminal: React.FC<FaultyTerminalProps> = ({
    ...
    const handleMouseMove = useCallback((event: MouseEvent) => {
        // We listen to window events now, so we can calculate relative to window
    ...
    useEffect(() => {
        ...
        const program = new Program(gl, {
            ...
            uniforms: {
                ...
                uTint: { value: new Vec3(tintVec[0], tintVec[1], tintVec[2]) },
        ...
        const resize = () => {
            if (!node || !rendererRef.current || !programRef.current) return;
            rendererRef.current.setSize(node.offsetWidth, node.offsetHeight);
            const canvas = rendererRef.current.gl.canvas;
            programRef.current.uniforms.iResolution.value = new Vec3(
                canvas.width,
                canvas.height,
        ...
        const loop = (time: number) => {
            ...
            if (pageLoadAnimation && loadStartRef.current === 0) {
                loadStartRef.current = time;
            }

            if (pause) {
                program.uniforms.iTime.value = timeRef.current;
            } else {
                const nextTime = (time * 0.001 + timeOffsetRef.current) * timeScale;
                program.uniforms.iTime.value = nextTime;
                timeRef.current = nextTime;
            }

            if (pageLoadAnimation && loadStartRef.current > 0) {
                const elapsed = time - loadStartRef.current;
                program.uniforms.uPageLoadProgress.value = Math.min(elapsed / 2000, 1);
            }

            if (mouseReact) {
                const current = smoothMouseRef.current;
        ...
        return () => {
            ...
            if (gl.canvas.parentElement === node) node.removeChild(gl.canvas);
